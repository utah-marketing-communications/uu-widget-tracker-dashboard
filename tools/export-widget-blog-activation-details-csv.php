<?php
/**
 * Export one row per active blog for each widget in the registry.
 *
 * Usage:
 * php tools/export-widget-blog-activation-details-csv.php \
 *   --input=/abs/path/Widget_Audit.registry.v1.csv \
 *   --output=/abs/path/Widget_Audit.report-blog-activation-details.v1.csv \
...
if ( empty( $args['input'] ) || empty( $args['output'] ) || empty( $args['map'] ) ) {
	uu_audit_cli_usage( 'tools/export-widget-blog-activation-details-csv.php', '--input=/path/input.csv --output=/path/output.csv --map=/path/multisite-map.json' );
	exit( 1 );
}
...
uu_report_each_widget_context(
	...
	function ( array $context ) use ( &$output_rows ) {
		foreach ( uu_report_widget_activation_rows( $context ) as $output_row ) {
...
usort(
	$output_rows,
	function ( $left, $right ) {
		$keys = array( 'Environment Label', 'Canonical Slug', 'Active Site Name' );
		foreach ( $keys as $key ) {
...
fwrite( STDOUT, 'Wrote widget blog activation detail CSV to ' . $output_file . ' (' . count( $output_rows ) . " rows)\n" );
