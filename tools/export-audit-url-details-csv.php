<?php
/**
 * Export one row per matched URL from remote UU Usage Tracker endpoints.
 *
 * Usage:
 * php tools/export-audit-url-details-csv.php \
 *   --input=/abs/path/AWS_Plugin_Audit.csv \
 *   --output=/abs/path/AWS_Plugin_Audit.url-details.csv \
...
if ( empty( $args['input'] ) || empty( $args['output'] ) || empty( $args['map'] ) ) {
	uu_audit_cli_usage( 'tools/export-audit-url-details-csv.php', '--input=/path/input.csv --output=/path/output.csv --map=/path/multisite-map.json' );
	exit( 1 );
}
...
uu_report_each_plugin_context(
	...
	function ( array $context ) use ( &$detail_rows ) {
		foreach ( uu_report_plugin_matched_url_rows( $context ) as $detail_row ) {
...
fwrite( STDOUT, 'Wrote audit URL detail CSV to ' . $output_file . ' (' . count( $detail_rows ) . " rows)\n" );
